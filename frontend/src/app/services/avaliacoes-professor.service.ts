...
export class AvaliacoesProfessorService {
  ...
  public criarAvaliacaoDoProfessor(avaliacao: AvaliacaoTurma): Observable<any> {
    return this.http.post(`${environment.API}/feedback-professor`, avaliacao,{ responseType: 'text' })
  }

  public editarAvaliacaoDoProfessor(avaliacao: AvaliacaoTurma): Observable<any> {
    return this.http.put(`${environment.API}/feedback-professor`, avaliacao,{ responseType: 'text' });
  }

  public listarAvaliacoesDoProfessor(professorId: number): Observable<any> {
    return this.http.get(`${environment.API}/feedback-professor/professor/${professorId}`)
  }

  public deletarAvaliacao(avaliacaoId: number): Observable<any> {
    return this.http.delete(`${environment.API}/feedback-professor/${avaliacaoId}`,{ responseType: 'text' })
  }
