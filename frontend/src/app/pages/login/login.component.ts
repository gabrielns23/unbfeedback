...
export class LoginComponent implements OnInit {
  ...
  onSubmit() {
    if (this.formLogin.valid) {
      ...
      const user:any= {};
      user.register = this.formLogin.value.matricula
      user.password = this.formLogin.value.senha
      console.log(user)
      this.service.autenticar(user).pipe(catchError(error => {
        ...
        localStorage.setItem("MATRICULA", value.register)
